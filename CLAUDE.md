...
- **Compiler Plugin** (`src/plugins/compiler/`) - Compiles C code using agbcc via `CCompiler` wrapper
...
- **CCompiler** (`src/shared/c-compiler/`) - Wrapper for the agbcc compiler toolchain

### Configuration

Configuration is YAML-based (`mizuchi.yaml`):

- `global` section: Pipeline-wide settings (`maxRetries`, `outputDir`, `contextPath`, `promptsDir`)
- `plugins` section: Per-plugin configuration (e.g., `claude-runner.projectPath`, `compiler.flags`)

Plugins validate their config using Zod schemas (e.g., `claudeRunnerConfigSchema`, `compilerConfigSchema`).
